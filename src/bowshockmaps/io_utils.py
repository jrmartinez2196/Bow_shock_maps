"""Loading and validation of per-source physical parameter files."""
...
from bowshockmaps.constants import pc
...
def get_source_params(source_name, params_dir=SYSTEMS_DIR):
    """
...
def validate_params(params):
    ...
    # Set defaults for optional parameters *before* range-checking them,
    # otherwise a missing key raises a KeyError instead of falling back.
    if "inclination" not in params:
        params["inclination"] = 60.0
        logger.info("'inclination' not found in file. Using default: %s°", params["inclination"])

    if "PA" not in params:
        params["PA"] = 90.0
        logger.info("'PA' not found in file. Using default: %s°", params["PA"])
