...
from bowshockmaps.constants import Rayleigh, c, h, kB, me, mec2, qe, sr_per_arcsec2


def emissivity_Halpha(n, T, ion_H=1.0):
    """
    H_alpha [R cm^-1]
    Mackey+ 2013
    """
...
def emissivity_OIII(n, T, ion_H=1.0, ion_O=1.0):
    """
...
def nu_emissivity_freefree(n, T, ion_H, Z_q, nu, gaunt_lookup):
    ...
    j = (
        C_j * (n_arr * ion_H) ** 2 * exp_factor * gaunt / np.sqrt(T_arr) / sr_per_arcsec2
    )  # erg/s/cm^3/arcsec^2/Hz
...
def precompute_gaunt_for_temperatures(nu_ff, Z):
    ...
    """
    from bowshockmaps.physics.gaunt_factor import gaunt_ff_calc
...
def nu_emissivity_sync(k0, B, p_inj, nu):
    ...
    cte_s = (
        qe**3.0
        / (h * mec2)
        * (3.0 * h * qe / (4.0 * np.pi * me**3.0 * c**5.0)) ** ((p_inj - 1.0) / 2.0)
    )

    j_syn = (
        a_syn * cte_s * k0 * B ** ((p_inj + 1.0) / 2.0) * eps ** (-(p_inj - 1.0) / 2.0) * eps / nu
    )  # erg/s/cm^3/sr/Hz
...
def cte_syn(p):
    ...
    a_num = (
        2.0 ** ((p - 1.0) / 2.0)
        * np.sqrt(3)
        * gamma((3.0 * p - 1.0) / 12.0)
        * gamma((3.0 * p + 19.0) / 12.0)
        * gamma((p + 5.0) / 4.0)
    )
    a_den = 8.0 * np.sqrt(np.pi) * (p + 1.0) * gamma((p + 7.0) / 4.0)
